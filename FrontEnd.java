...
public class FrontEnd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher error = request.getRequestDispatcher("/Error.html");
		 ...
		 try {
					if(game_params.equals("ColoredPong")){color_pong.forward(request, response);
					}else
					if(game_params.equals("DodgeRacing")){dodge_challenge.forward(request, response);
					}else
					if(game_params.equals("StarShipBattle")){neutral_zone.forward(request, response);
					}else {
